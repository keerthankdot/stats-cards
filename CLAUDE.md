...
**Follow-up — `interruption.mp4` restructured to a fixed 4s sequential timeline (not a faithful-timing copy anymore):** user asked to slow it down and make it exactly 4s, with the wave/graph draw and the counter as two distinct back-to-back phases (0–2s graph, 2–4s number) instead of the source's overlapping timing (`animateLines`: counter starts 200ms into the 1400ms wave draw, both finish within ~1.6s total). This one intentionally deviates from the "replay the site's real animation constants" approach used for the other 5 — it's a paced-for-video request, not a bug fix, so `final-flow.html` remains untouched; only the standalone capture script's timing changed. Split `animateLines` into two independent functions (`animateWave` — just the clip-rect reveal + line stroke-dashoffset, no counter; `animateCounter` — just `countToLinear`, called separately) and drove them from Node with explicit trigger times instead of letting one function schedule both internally. Also had the counter's own internal duration run 150ms short of its allotted window (1850ms within the 2000ms slot) so the final value (`-25%`) is fully settled and held for a few frames before the 4s cutoff — without that buffer the last captured frame landed mid-count (`-24%`) since the loop breaks the instant elapsed time passes the target, not on animation-complete.
